...
const Cartpage = () => {
  ...
  const fetchBookings = async () => {
    const token = localStorage.getItem("authToken");
    if (!token) return setError("No user found. Please log in.");
    try {
      const { data } = await axios.get("https://cleanease-backend-2-1.onrender.com/api/Cartpage", {
        headers: { Authorization: `Bearer ${token}` },
      });
      setBookings(data.bookings);
      dispatch(setBookingCount(data.bookings.length));
    } catch (err) {
      setError(err.response?.data?.message || "Error fetching bookings.");
    }
  };
  ...
  const removeBooking = async (bookingId) => {
    ...
    if (!token) return setError("No user found. Please log in.");
    try {
      await axios.delete("https://cleanease-backend-2-1.onrender.com/api/removeBooking", {
        headers: { Authorization: `Bearer ${token}` },
        data: { bookingId },
      });
  ...
  return (
    <div className="Cart">
      ...
      {bookings.length > 0 ? (
        <motion.ul className="booking-list" variants={listVariants} initial="hidden" animate="visible">
          {bookings.map((booking) => (
            <motion.li key={booking._id} className="booking-item" variants={itemVariants}>
              <h3>{booking.employeeName}</h3>
              <img src={booking.employeeImage} alt={booking.employeeName} width="100" />
              <p>City: {booking.city}</p>
              <p>Date: {new Date(booking.date).toLocaleDateString()}</p>
              <p>Time: {booking.time}</p>
              <button onClick={() => removeBooking(booking._id)} className="remove-button">Remove Booking</button>
            </motion.li>
          ))}
        </motion.ul>
      ) : <p>No bookings available.</p>}
